...
namespace FilmBookmarkService.Core.CloudFlareUtilities
{
    ...
    /// </remarks>
    internal class ClearanceHandler : DelegatingHandler
    {
        ...
        private void InjectCookies(HttpRequestMessage request)
        {
            ...
            {
                request.Headers.Add(Cookie, idCookie.ToHeaderValue());
                request.Headers.Add(Cookie, clearanceCookie.ToHeaderValue());
            }
        }
        ...
        private async Task PassClearance(HttpResponseMessage response, CancellationToken cancellationToken)
        {
            ...
            if (response.RequestMessage.Headers.TryGetValues(UserAgent, out userAgent))
                clearanceRequest.Headers.Add(UserAgent, userAgent);
